...
let pidLocal = return (0., 0., 0., 0.) (* sum, deriv, control *)

(* time, mesure, reference, (sum, deriv, derivMesure, control) *)
let pid = 
  let facProp   = 0.5 in
  let facInteg  = 0.3 in
  ...
  let proposal 
    (t1, m1, r1, (s1, d1, dm1, c1)) 
    (t2, m2, r2, (s2, d2, dm2, c2)) 
    =
    let p = prop m2 r2 in
    let dt = (float_of_int (t2 - t1)) in
    if dt = 0. then
      	let c3 = ctrl p s1 d1 in 
        (t1, m2, r2, (s1, d1, dm1, c3))
    else
      let s3 = integ m2 r2 s1 in
      let d3 = deriv dm1 m2 dt in
      let c3 = ctrl p s3 d3 in
      (t2, m2, r2, (s3, d3, m2, c3))
  in
...
let pidMesureServer, _ = 
  ...
  Network.stream_unix_server
    ~no_fork:()
    ~socketfile:"/tmp/pidscine/mesure"
    ~protocol
    ()
...
let pidReferenceServer, _ = 
  ...
  Network.stream_unix_server
    ~no_fork:()
    ~socketfile:"/tmp/pidscine/reference"
    ~protocol
    ()
...
let pidControlClient =
  ...
  Network.stream_unix_client
    ~socketfile:"/tmp/pidscine/control"
    ~protocol
    ()
